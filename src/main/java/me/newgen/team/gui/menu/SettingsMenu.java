...
public final class SettingsMenu extends Menu {
    ...
    private MenuLayoutManager.MenuLayout layout() {
        return plugin.menuLayouts().layout("settings");
    }

    @Override protected int size() { return layout().size(45); }
    @Override protected String title() {
        Team t = plugin.teams().byPlayer(viewer.getUniqueId());
        return layout().title(t == null ? L("gui.settings.title") : L("gui.settings.title-team", "team", t.name()));
    }
    ...
    protected void build() {
        ...
        toggle(team, canEdit, l, "open-team", 10, Material.IRON_DOOR, L("gui.settings.open-team"),
                L("gui.settings.open-team-desc"), () -> s.open, v -> s.open = v);
        toggle(team, canEdit, l, "friendly-fire", 11, Material.IRON_SWORD, L("gui.settings.friendly-fire"),
                L("gui.settings.friendly-fire-desc"), () -> s.friendlyFire, v -> s.friendlyFire = v);
        toggle(team, canEdit, l, "team-chat", 12, Material.PAPER, L("gui.settings.team-chat"),
                L("gui.settings.team-chat-desc"), () -> s.teamChat, v -> s.teamChat = v);
        toggle(team, canEdit, l, "auto-accept", 13, Material.WRITABLE_BOOK, L("gui.settings.auto-accept"),
                L("gui.settings.auto-accept-desc"), () -> s.autoAcceptInvite, v -> s.autoAcceptInvite = v);

        toggle(team, canEdit, l, "public-join", 19, Material.OAK_SIGN, L("gui.settings.public-join"),
                L("gui.settings.public-join-desc"), () -> s.publicJoin, v -> s.publicJoin = v);
        toggle(team, canEdit, l, "join-notify", 20, Material.BELL, L("gui.settings.join-notify"),
                L("gui.settings.join-notify-desc"), () -> s.notifyJoinLeave, v -> s.notifyJoinLeave = v);
        toggle(team, canEdit, l, "ally-requests", 21, Material.EMERALD, L("gui.settings.ally-requests"),
                L("gui.settings.ally-requests-desc"), () -> s.allyRequests, v -> s.allyRequests = v);

        set(l.slot("back", 40), Icons.back(), e -> { Sounds.click(viewer); plugin.menus().openMain(viewer); });
        set(l.slot("close", 44), Icons.close(), e -> { Sounds.click(viewer); viewer.closeInventory(); });
    }

    private void toggle(Team team, boolean canEdit, MenuLayoutManager.MenuLayout l, String key,
                        int defSlot, Material icon, String name,
                        String desc, Predicate0 getter, Consumer<Boolean> setter) {
        if (!l.enabled(key)) return;
        boolean state = getter.get();
        set(l.slot(key, defSlot), ItemBuilder.of(state ? Material.LIME_DYE : Material.GRAY_DYE)
