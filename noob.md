...
- Kế hoạch refactor premium theo phase: 1 Rebrand (XONG) → 2 Theme+Language (XONG) → 3 Config split (XONG 2026-07-05, build xanh) → 4 Storage đa DB (XONG 2026-07-05, build xanh) → 5 GUI config-driven → 6 API/Events/Hooks/Placeholder → 7 Reload+Logging+README.
- Phase 3: config tách thành config.yml (chỉ language) + settings.yml + levels.yml + homes.yml + chest.yml + database.yml, comment song ngữ. `ConfigManager.loadYaml()` copy default từ jar lần đầu chạy. `messages.yml` cũ ĐÃ XÓA.
- Phase 4: `SqliteStorage` → `JdbcStorage` (SQLITE/MYSQL/MARIADB/POSTGRESQL chọn qua database.yml, `DatabaseSettings` parse + fallback SQLITE). MySQL/MariaDB dùng chung driver mariadb (`jdbc:mariadb://`). Shade + relocate mariadb-java-client 3.5.1 & postgresql 42.7.4. SQLite giữ pool size 1 (tránh lock). Upsert SQL tách theo dialect (ON DUPLICATE KEY vs ON CONFLICT).
